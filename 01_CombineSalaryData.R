...
saldat <- salary.data.clean %>%
  mutate(Player = ifelse(is.na(First), Last,
                         ifelse(is.na(Last), First,
                                ifelse(First == Last, 
                                       First,
                                       trimws(paste0(First, " ", Last))))),
