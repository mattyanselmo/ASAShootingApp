...
alldata <- shooting %>%
  ...
  right_join(minutesPlayed %>%
               select(-X), 
            by = c('gameID', 'player')) %>%
  left_join(touches %>%
              select(gameID, player, touches, date, team), 
            by = c('gameID', 'player')) %>%
  mutate_each(funs(((function(x) {ifelse(is.na(x), 0, as.numeric(x))})(.))), -c(gameID, shooter, meddist, meddist.pass, player, date, team)) %>%
  ungroup() %>%
  filter(!is.na(date))
...
nofkpkdata <- shooting %>%
  mutate(date = as.Date(date, format = '%m/%d/%Y')) %>%
  filter(!(patternOfPlay %in% c('Free kick', 'Penalty'))) %>%
  group_by(gameID, shooter) %>%
  summarize(shots = n(),
            unassisted = shots - sum(assisted),
            meddist = median(distance),
