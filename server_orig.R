  ...
  dt_total <- reactive({
    ...
    if(shooter_inputs$seasonordate() == 'Season'){
      dt_total <- shooterxgoals.func(playerxgoals,
                                     ...
                                     minfilter = shooter_inputs$minfilter(),
                                     shotfilter = shooter_inputs$minshots(),
                                     keyfilter = shooter_inputs$minkeypasses(),
                                     teamfilter = shooter_inputs$teamfilter(),
    ...
    if("Pos" %in% names(dt_total)){
      dt_total %>% filter(Pos %in% shooter_inputs$position() | is.na(Pos))
    } else{
      dt_total
    }
  })
