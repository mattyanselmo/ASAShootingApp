# By Eliott McKinley
library(ggplot2)
createPitch <- function(xmax=10516, ymax=7315, grass_colour="white", line_colour="gray", background_colour="white", goal_colour="gray", thirty_yard=0){
  ...
  ## create 30
  thirty=circleFun(center=c(xmax, ymax/2), diameter=thirtyL, npoints = 1000)
  ...
  p <- ggplot() + xlim(c(-1,xmax+1)) + ylim(c(-1,ymax+1)) + 
    # add the theme 
    theme_blankPitch() +
  ...
  if (thirty_yard==1){
    p=p+geom_path(data=thirty, aes(x=x,y=y), colour = "gold3")
  }
